...
点击我的并抓包，
抓取https://cluster.qifeixian.com/api/user/v1/center/info，此链接请求头的x-ds-key
变量export xtkp='x-ds-key'
多账号@隔开
export xtkp='x-ds-key@x-ds-key'
*/
// corn 0 9 * * *
...
let xtkp = ($.isNode() ? process.env.xtkp : $.getdata("xtkp")) || ""
let xtkpArr = [];
let data = '';
let msg = '';
...
let referer = 'https://www.qifeixian.com/'
//-----------------------------------------------------------------------
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        if (!(await Envs()))
            ...
            addNotifyStr(`\n=================== 共找到 ${xtkpArr.length} 个账号 ===================`)
            if (debug) {
                log(`【debug】 这是你的全部账号数组:\n ${xtkpArr}`);
            }
            for (let index = 0; index < xtkpArr.length; index++) {
...
async function task() {
    return new Promise((resolve) => {
        ...
        var options = {
            method: 'GET',
            url: 'https://cluster.qifeixian.com/api/activity-center/v1/rabbit/task?type=20',
            headers: {
                Host: host,
                Connection: connection,
                'x-ds-key': newtoken,
            ...
            try {
                ...
                if (array) {
                    for (let i = 0; i < array.length; i++) {
                        addNotifyStr('taskId:' + data.data[i].taskId + ' title:' + data.data[i].title + ' finish:' + data.data[i].finish)
                    }
...
async function daySign() {
    return new Promise((resolve) => {
        ...
        var options = {
            method: 'POST',
            url: 'https://cluster.qifeixian.com/api/activity-center/v1/rabbit/finish/task',
            headers: {
                Host: host,
                Connection: connection,
                'x-ds-key': newtoken,
        ...
        axios.request(options).then(async function (response) {
            try {
                ...
                if (data.code == 10000) {
                    addNotifyStr('每日签到：' + data.data.message)
                } else {
                    addNotifyStr(data.msg)
                }
    ...
    return new Promise((resolve) => {
        ...
        axios.request(options).then(async function (response) {
            try {
                ...
                if (data.code == 10000) {
                    addNotifyStr('每日首次浏览爆款商品15s：' + data.data.message)
                } else {
                    addNotifyStr(data.msg)
                }
    ...
    return new Promise((resolve) => {
        ...
        axios.request(options).then(async function (response) {
            try {
                ...
                if (data.code == 10000) {
                    addNotifyStr('每日首次浏览商城商品15s：' + data.data.message)
                } else {
                    addNotifyStr(data.msg)
                }
...
async function Envs() {
    if (xtkp) {
        if (xtkp.indexOf("@") != -1) {
            xtkp.split("@").forEach((item) => {

                xtkpArr.push(item);
            });
        } else if (xtkp.indexOf("\n") != -1) {
            xtkp.split("\n").forEach((item) => {
                xtkpArr.push(item);
            });
        } else {
